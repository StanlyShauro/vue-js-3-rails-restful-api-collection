<template>
  ...
  <table class="table table-striped table-hover border">
    ...
    <tbody>
      <editable-list-row v-for="item in items" :key="JSON.stringify(item.id)"
        :item='item'
        :fields='fields'
        @click="editItem(item)"
      >
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadItemsList = async () => {
      items.value = await fetch(props.list_url + ".json").then((response) => response.json());
    }
    const newItem = async () => {
      let new_item = {}
      props.fields.forEach((field) => { new_item[field.name] = '' });
